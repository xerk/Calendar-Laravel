...
class BookingCalendarRescheduleResource extends JsonResource
{
    /**
     * @var
     */
     ...
     * @return void
     */
    public function __construct($resource, $currentBooking)
    {
    ...
    public function toArray($request)
    {
        ...
        return [
            ...
            'bookings' => Booking::where('id', '<>', $this->currentBooking->id)->where('calendar_id', $this->id)->where('cancelled_at', null)->get(),
    ...
    {
        if (!$request->range_start || !$request->range_end || !$request->timezone) {
            return [];
        }
        ...
        $availabilityDays = [];
        $pluckDate = collect($days)->where('enabled', true)->where('status', 'available')->pluck('date')->flatten(1)->toArray();
        foreach ($pluckDate as $key => $value) {
            $availabilityDays[] = (int)Carbon::parse($value)->format('d');
        }
        ...
        if ($this->is_isolate) {
            ...
            $bookings = Booking::where('id', '<>', $this->currentBooking->id)->where('user_id', $this->user_id)->where('cancelled_at', null)->get();
        }
                ...
                if ($day['enabled'] && count($day['spots'])) {
                    foreach ($day['spots'] as $spotKey => &$spot) {
                        if (Carbon::parse($booking->date_time)->setTimezone($userTimezone)->format('Y-m-d') == $day['date']) {
                            ...
                            // dd($bookingStart, $bookingEnd, $spotStart, $spotEnd);

                            // $spot['data'] = [
                            //     'bookingStart' => $bookingStart->format('Y-m-d H:i:s'),
                            //     'bookingEnd' => $bookingEnd->format('Y-m-d H:i:s'),
                            //     'spotStart' => $spotStart->format('Y-m-d H:i:s'),
                            //     'spotEnd' => $spotEnd->format('Y-m-d H:i:s'),
                            // ];
        ...
        foreach ($slots as $slot) {
            ...
            foreach ($period as $date) {
                ...
                $spots[] = [
                    ...
                    'former_text' => Carbon::parse($this->currentBooking->date_time)->equalTo($date) ? 'Former Time' : null,
                    'former_status' => Carbon::parse($this->currentBooking->date_time)->equalTo($date) ? true : false,
